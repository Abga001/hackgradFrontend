...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchRecentMessages = async () => {
      try {
        // Wrap the subscription in a try-catch
        unsubscribe = messageService.subscribeToUserConversations(
          currentUser._id,
          async (conversations) => {
            try {
              // Your existing logic for processing conversations
              ...
              console.error('Error processing conversations:', processingError);
              setMessageError('Failed to process conversations');
            }
          }
        );
      } catch (subscriptionError) {
        console.error('Subscription error:', subscriptionError);
        setMessageError('Failed to subscribe to conversations');
      }
    };
